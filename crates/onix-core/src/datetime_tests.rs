...
fn date_from_ordinal_inverts_ordinal_across_the_python_range() {
    for ordinal in [1_i64, 719_163, 738_886, 3_652_059] {
        assert_eq!(Date::from_ordinal(ordinal).ordinal(), ordinal);
    }
    assert_eq!(Date::from_ordinal(738_886), date(2024, 1, 1));
    assert_eq!(Date::from_ordinal(1), date(1, 1, 1));
}
