...
fn an_unnormalizable_datetime_under_ignore_order_is_reported_raw() {
    ...
    let list = |items: Vec<CValue>| carr(items);
...
/// The class-name check on an LCS `'replace'`-opcode pairwise comparison
/// ([`crate::diff::array::insert_lcs_pair_finding`]'s own class-name check,
/// distinct from `diff_at`'s dispatch-level check the tests above exercise):
...
fn a_datetime_subclass_inside_an_lcs_replace_pair_is_a_type_change() {
    ...
    let a = carr(vec![subclass_a]);
    let b = carr(vec![base_b]);
...
fn a_sets_report_does_not_depend_on_its_member_order() {
    let float_tuple = ctuple(vec![ctup(&[json!(1.0)])]);
    let int_tuple = ctuple(vec![ctup(&[json!(1)]), cv(&json!(0))]);
    let b = CValue::Set(SetItems::new(vec![ctuple(vec![ctup(&[
        json!(1),
        json!(1),
    ])])]));
