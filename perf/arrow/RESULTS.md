...
scalar Arrow type the row diff hashes or renders, at the same ~5 GB-per-side target as the narrow
fixture, so far fewer, much wider rows — see that script's module docstring for the full column
set, mutation mix, and the two Parquet-format gaps (`interval`, `date64`) it works around.

| Size | Rows (`a`) | `a.parquet` | `b.parquet` | `a` SHA-256 | `b` SHA-256 |
|---|---|---|---|---|---|
| 1M | 1,000,000 | 224.5 MB | 225.0 MB | `edfdbe0e6f2b328e0c5eeaf4f22794ee3e53bc2a191d2c1c6a772d58e3491562` | `2b0e29827e2ffc379de8cd53ea7ef693683d7eaed25ddea21b2362d61d00b39c` |
| full | 22,261,000 | 4,999.2 MB | 5,008.5 MB | `20d49e1e7c79d2cfec646995df543180250229ff7ac21b99f606132c1a0633e9` | `da1c435e3c6e272eaf6421e8ac2f871b7801aba91bab6e281c067a806e3bb92e` |

Measured 2026-09-06T04:44Z, same machine and environment as the narrow section's table above,
against `deepdiff-rs` built at 0.9.2, immediately before this branch rebased onto main's 0.9.3
(#82: a Python lone-surrogate fix with no `onix-arrow` changes, confirmed by diff).
...
1,161,541 changed cells vs. 171,541 for DuckDB/polars; full reports 222,610 added, 222,610 removed,
`onix` 25,855,867 changed cells vs. 3,817,477 for DuckDB/polars, 0 duplicate keys at both sizes.
...
| --- | --- | --- | --- |
| onix (`diff_tables`) | 5.262 s | 5.973 s | 2890.4 MB |
| DuckDB (oracle SQL) | 340.19 ms | 3.481 s | 2586.5 MB |
| polars (anti-join/inner-join) | 134.24 ms | 1.047 s | 1454.0 MB |

#### Full pair (22.261M rows, ~5 GB + ~5 GB)

| Tool | Wall clock (median) | CPU seconds (median) | Peak RSS (median) |
| --- | --- | --- | --- |
| onix (`diff_tables`) | 194.781 s | 209.121 s | 55103.7 MB |
| DuckDB (oracle SQL) | 5.103 s | 70.496 s | 12294.4 MB |
| polars (anti-join/inner-join) | 3.248 s | 27.776 s | 28114.5 MB |
...
0% CPU throughout, per the same convention the narrow environment note above uses).

`onix`'s wide numbers are far past its narrow ones (19.2 s → 194.8 s wall, a ~10x jump despite
22.26M rows being fewer than narrow's 37M), because `ts_cast`'s zone-awareness change makes nearly
every surviving row "changed" (see the generator's module docstring): the cell-diff pass compares
all 27 non-key columns for essentially all 22M rows, not the small mutated fraction narrow's
fixture produces.
This is the fixture doing its job — "every rendering rule on the hot path" — not a regression
against the narrow number above. DuckDB's and polars' wall clock stays low because both parallelize
that same column-by-column comparison across this machine's cores, the same single- vs.
multi-threaded gap the narrow section above describes, now paid over far more compared cells.
Peak RSS also grows with it: `onix`'s materialize-and-render pass holds the changed rows' cells as
owned strings (`row_diff.rs`'s module doc), and here that is nearly the whole table's non-key
columns at once.

## Disk usage

Both fixture pairs (narrow and wide) at both sizes, all resident at once, from the two "Fixture
pair" tables above: narrow 1M (269.4 MB) + narrow full (9,970.9 MB) + wide 1M (449.5 MB) + wide
full (10,007.7 MB) — about 20.7 GB, plus `bench_raw/`'s per-run JSON files (under 1 MB total,
measured at 192 KB for the wide runs alone). Nothing under `perf/arrow/fixtures/` or
`perf/arrow/bench_raw/` is committed.
