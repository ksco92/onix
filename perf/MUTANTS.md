...
`make mutants` enumerates a deterministic **1211** mutants (20 in `onix-cli`,
980 in `onix-core`, 211 in `onix-arrow`). A standalone `cargo mutants -p
onix-arrow` on a quiet machine reports, for the 211 `onix-arrow` mutants
(144 in `row_diff.rs`, 38 in `schema.rs`, 17 in `table_diff.rs`, 5 in `lib.rs`,
4 in `error.rs`, 3 in `options.rs`): **162 caught, 39 unviable, 9 timeout, 1
...
them); with `onix-arrow`'s 211 the top-line total is 1211, as recorded above. The last full
