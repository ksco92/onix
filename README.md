...
- Ordered sequences (`list` or `tuple`) of scalars (null, bool, number, string, datetime, date) run a `difflib`-style `O(N*M)` matcher on the default path with its popular-element (autojunk) purge disabled for `DeepDiff` parity, worst for mostly-repeated values with scattered edits (about 240 s for two 8,000-element lists of two repeated values with every tenth element changed, growing faster than quadratically, versus about 26 s at 100,000 distinct values), so bound the size of untrusted sequences yourself. See [`crates/onix-core/src/lcs.rs`](crates/onix-core/src/lcs.rs).
