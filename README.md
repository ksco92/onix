...
- Supported value types are `None`, `bool`, `int`, `float`, `str`, `dict`, `list`, `tuple`, `set`, `frozenset`, `datetime.datetime`, `datetime.date`, `datetime.time`, and `datetime.timedelta`; a `set`/`frozenset` member may be any of these except a `list`, `dict` or `set`, matching Python's own hashability rule, transitively through whatever the member nests. `int`s must fit in `i64`/`u64`, `float`s must be finite, and anything else — a custom object, an arbitrary-precision `int`, or a non-finite `float` — raises `TypeError`/`ValueError` naming the exact path it was found at. The **Datetimes** and **Sets** bullets below cover the deliberate divergences for those types. See [`crates/onix-py/src/convert.rs`](crates/onix-py/src/convert.rs) and [`tests/golden/README.md`](tests/golden/README.md).
- A `dict` key may be `str`, `None`, `bool`, `int`, `float`, `datetime.datetime`, `datetime.date`, or a `tuple` of those (never a nested `tuple`, and never a subclass of any of these — see the bullet below). A non-`str` key's path renders via Python's own `repr()`, except a `tuple` key, which splits into one bracket group per element (`root[1][2]` for the key `(1, 2)`, never `root[(1, 2)]`) — DeepDiff's own rendering rule, with one deliberate exception: DeepDiff's own path rendering for the empty tuple `()` as a key is a real upstream bug (the path comes back `None`), which onix does not reproduce, rendering `root[]` instead (see [`tests/golden/README.md`](tests/golden/README.md)). `1`, `1.0`, and `True` are the same *key* between two dicts being compared (Python `dict`/`set` equality), matching DeepDiff. A key of any other type raises `TypeError` naming the key's type and the path to the dict containing it.
