...
- Supported value types are `None`, `bool`, `int`, `float`, `str`, `dict` (with `str` keys), `list`, `tuple`, `set`, `frozenset`, `datetime.datetime`, and `datetime.date`; `int`s must fit in `i64`/`u64`, `float`s must be finite, and anything else (`time`, `timedelta`, custom object, non-`str` dict key) raises `TypeError` or `ValueError` naming the exact path it was found at. A `set`/`frozenset` member is restricted further, to `None`, `bool`, `int`, `float`, `str`, `tuple` and `frozenset`, and the restriction is *transitive*: a `list`, `dict`, `set`, `datetime` or `date` anywhere inside a set member is refused too (calendar values there land with issue #21). See [`crates/onix-py/src/convert.rs`](crates/onix-py/src/convert.rs).
