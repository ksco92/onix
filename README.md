...
- Supported value types are `None`, `bool`, `int`, `float`, `str`, `dict` (with `str` keys), `list`, `tuple`, `set`, `frozenset`, `datetime.datetime`, and `datetime.date`, and a subclass of any of the last seven (a `list`/`tuple`/`set`/`frozenset`/`dict` subclass, a `namedtuple`, or a `datetime`/`date` subclass such as pandas' `Timestamp`); a `set`/`frozenset` member may be any of the base types except a `list`, `dict` or `set` (matching Python's own hashability rule, transitively through whatever the member nests) plus a `datetime`/`date` subclass, but not a `tuple`/`frozenset` subclass or a `namedtuple`. `int`s must fit in `i64`/`u64`, `float`s must be finite, and anything else — `time`, `timedelta`, a non-`str` dict key, a custom object, an arbitrary-precision `int`, or a non-finite `float` — raises `TypeError`/`ValueError` naming the exact path it was found at. The **Datetimes** and **Sets** bullets below cover the deliberate divergences for those two types. See [`crates/onix-py/src/convert.rs`](crates/onix-py/src/convert.rs) and [`tests/golden/README.md`](tests/golden/README.md).
- A subclass instance compares as its base type but carries its own class name into a `type_changes` entry, matching DeepDiff, which reports every value under `type(obj).__name__`: a subclass-vs-base pair is a type change even when every field matches. A `namedtuple` diffs positionally (`root[0][0]`), not by field the way DeepDiff does (`root[0].x`) — a documented, permanent divergence, not an approximation of it. A `type_changes` entry's `old_type`/`new_type` are type *names* in `to_dict()`, where DeepDiff returns the type objects. Both are described in [`tests/golden/README.md`](tests/golden/README.md).
