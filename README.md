...
- A `namedtuple` is diffed as the tuple it subclasses (by index), where DeepDiff walks its fields (`root[0].x`); and a `type_changes` entry's `old_type`/`new_type` are type *names* in `to_dict()`, where DeepDiff returns the type objects. See [`tests/golden/README.md`](tests/golden/README.md).
