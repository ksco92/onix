...
- Supported value types are `None`, `bool`, `int`, `float`, `str`, `dict` (with `str` keys), `list`, `tuple`, `set`, `frozenset`, `datetime.datetime`, and `datetime.date`; a `set`/`frozenset` member may be any of these except a `list`, `dict` or `set`, matching Python's own hashability rule, transitively through whatever the member nests. `int`s must fit in `i64`/`u64`, `float`s must be finite, and anything else — `time`, `timedelta`, a non-`str` dict key, a custom object, an arbitrary-precision `int`, or a non-finite `float` — raises `TypeError`/`ValueError` naming the exact path it was found at. Three divergences from DeepDiff are deliberate: sets diff and render in a deterministic order, where DeepDiff's own order depends on the running process's set iteration (hash order, and `PYTHONHASHSEED` for `str` members); a report holding a `frozenset` or a `date` value still serializes to JSON here, where DeepDiff's own `to_json()` raises `TypeError` on either — a documented superset, not a difference in the findings; and a `zoneinfo`/`pytz` `tzinfo` comes back from `to_dict()` as a fixed-offset `datetime.timezone` carrying the offset in force when it was converted, not the original zone object. See [`crates/onix-py/src/convert.rs`](crates/onix-py/src/convert.rs) and [`tests/golden/README.md`](tests/golden/README.md).
