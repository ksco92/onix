...
- `diff_tables` row diff: per-row hash (32 B/row/side single-threaded): 75 MB at 1M, 660 MB at 10M, linear in row count; wall time `N·log N` across workers. The parallel path keeps the 32 B hash (plus an 8 B count) for the left side only, and a 32 B map entry per distinct right key absent from the left (~70 MB for 1M such keys): 600/583 MB at 8M (1/18 threads), 2.7/2.5 GB at 37M; `threads=1` sequential. Duplicate-key report (every distinct duplicated key): 40 MB (16 B keys) to 0.82 GB (1 KB keys) at 200k (100k distinct), 197 MB at 1M (500k distinct, 16 B keys). Right-side duplicate keys, parallel path: the first right row of a key the left lacks is held at full width until the right repeats the key, and the first right row of a key the left holds once with a different row hash is spilled even when a later batch repeats it (1.03 GB of spill and +0.86 GB peak for 500k such keys with 2 KB of value columns, 1M rows/side). Cell-pass resident (spilled rows-x-column-width, both sides + 2x `cells_changed` output + hash vectors) at 18 threads: 0.47/0.96/4.71 GB at 100k/200k/1M output-dominated (single-threaded 8.07 GB); eight 512 B columns, one differing (150k changed cells, ~160 MB output): 1.68 GB at 150k spill-dominated vs 0.54 GB one column; 0.35 GB at 1M two int64. A row kept for `rows_added`, `rows_removed` or the duplicate-key report keeps its input batch's byte-view data resident, since the output carries those buffers whole: one batch's view data per batch with a kept row, per side (100 removed rows spread over a 1M-row side with two 1 KB `Utf8View` columns hold the side's ~2.0 GB of view data, as on 0.11.2).
- Temp disk, same call: each input is spooled to an anonymous file (`tempfile`: unlinked at once, mode 0600, no name, nothing left on abnormal exit), since a side may be read more than once (the right once and the left twice on the parallel path, each three times single-threaded). Both sides' changed rows, plus the first right row of each key the left holds once that a later right batch repeats, are spilled to anonymous per-partition files (the right's while it is hashed, the left's while it is re-read): byte-view columns cast to a large-offset type (buffers can exceed i32's ~2 GB), dictionaries decoded to value type, keeping the spill compact, independent of partition count (~97 GB at 64 threads undecoded vs ~32 GB decoded, wide pair). Both spools and both spills are resident together until the cell pass ends: ~23.8 GB of temp disk for the full wide pair (12.3 GB of input spool plus 11.5 GB of spill, the same at 2, 18 and 64 threads), 11.9 GB for the full narrow pair. Whole-process peaks (34-column fixture, 16.875M rows/side): ~29 GB at 18 threads, 40 GB at 2, 32 GB at 64, against 33, 46 and 34 GB on 0.11.2 and ~67 GB pre-streaming (Linux spill may be a RAM-backed `tmpfs`). A full temp filesystem raises `ValueError` naming `TMPDIR`.
- The size gate peeks up to 50,000 rows or 64 MB decoded per side, holding one whole producer batch per side beyond it: a 49,999-row/side, 8 KB-cell, 100-row-batch pair peaks ~68 MB at 18 threads, 12 MB single-threaded, ~1.6 GB single-threaded (1.2 GB at 18 threads) for one whole-side batch.
- None of the row-diff memory, temp-disk or size-gate-peek costs above has a built-in cap. Bound: row count; changed fraction; right-side duplicate keys; column widths (key-column width, for duplicate-heavy data; for the cell pass, the total width of all common value columns, since every one spills in full per changed row whether it changed or not, plus the rendered changed cells; any column's width for the size-gate peek, which buffers decoded cells whether or not they change); the producer's batch size; thread count, lowest at 18 threads (measured default), higher at 2 threads (two partitions each holding half the rows), slightly higher at 64: spill-dominated 2.87 GB at 2 threads, 1.96 GB at 64; output-dominated at 200k rows, 1.42 GB at 2, 1.26 GB at 64.
