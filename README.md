...
The row diff hashes and classifies rows across all cores by default. Pass `threads=` to override: `None` (the default) uses the machine's available parallelism, `1` runs single-threaded, and a value below 1 raises `ValueError`. The result is byte-identical at any thread count — same rows, same ordering, same `to_json()` — so the knob trades only speed for cores.
...
Both reports carry their full methodology, fairness rules, and the reproduce command. `perf/RESULTS.md` is an upper bound (JSON parsed straight into the engine, no Python-object conversion); the bindings table is the product-surface number. Regenerate them with `perf/run_bench.sh` and `crates/onix-py/benchmarks/bench_bindings.py` (see [CONTRIBUTING.md](CONTRIBUTING.md)). The Arrow table diff's cost against two hand-rolled baselines — a DuckDB SQL join and a polars anti-join/inner-join diff, on a seeded 5 GB parquet pair and its 1M-row subset — is in [`perf/arrow/RESULTS.md`](perf/arrow/RESULTS.md): the row diff now hashes and classifies across every core, so at the 5 GB size its wall time is a small multiple of the two parallelized baselines' rather than the several-fold gap the earlier single-threaded row diff showed; see the linked results for the figures and the before/after; regenerated with `perf/arrow/bench_tables.py`.
