...
- `diff_tables` row diff, per call: per-row hash (32 B/row/side, sorted): 75 MB at 1M rows/side, 660 MB at 10M; wall time `N·log N` across workers. Parallel overhead ~10-15 B/row/side: 629/857 MB at 8M (1/18 threads), 2.9/3.6 GB at 37M; `threads=1` is sequential. Duplicate-key report (every distinct duplicated key): 37 MB (16 B keys) to 1.05 GB (1 KB keys) at 200k (100k distinct), 165 MB at 1M (500k distinct). Cell-pass resident equals spilled changed rows (rows times common-column width, both sides) plus 2x `cells_changed` output plus hash vectors, spill dominates wide/few-cell rows, output dominates many-cell rows: 0.92/1.31/4.81 GB at 100k/200k/1M output-dominated (single-threaded 8.30 GB), 2.09 GB at 150k spill-dominated (0.84 GB one column), 0.38 GB at 1M two int64.
...
- No term is capped. Bound: row count; changed fraction; column widths (key columns for duplicate-heavy data; cell pass: common value-column width, since every column spills in full per changed row, plus changed cells; any column's width for the size-gate peek, which always buffers decoded cells); the producer's batch size; thread count (cell-pass peak lowest at default, higher at 2 threads and slightly higher at 64: spill-dominated 3.03 to 2.09 GB, output-dominated 1.98 to 1.49 GB at 200k). The size gate peeks each side up to 50,000 rows or 64 MB decoded, holding up to one batch beyond that bound: a 49,999-row/side, 8 KB-cell, 100-row-batch pair peaks ~78 MB at 18 threads, 14 MB single-threaded, ~1.6 GB as one whole-side batch.
