...
Type comparison uses the full logical Arrow type, including timestamp unit and timezone and decimal precision and scale; nullability is ignored (but reported in each record). Physical encodings that carry the same logical type are treated as equal, so the same table read through different libraries does not report spurious type changes — applied recursively through `List`/`Struct`/`Map`/`FixedSizeList` children: a dictionary-encoded type compares as its value type (dictionary-encoded string == plain string, `list<dictionary>` == `list<string>`), `Utf8View`/`LargeUtf8` as `Utf8`, `BinaryView`/`LargeBinary` as `Binary`, and `LargeList`/`ListView`/`LargeListView` as `List`. A list's element field name and nullability are dropped before comparing, because producers disagree on both (DuckDB names the element `l`, pyarrow `item`, Parquet `element`); a `FixedSizeList` is normalized the same way but keeps its width. An Arrow `Map` and the `LargeList<Struct<key, value>>` polars re-exports a map as are unified to one canonical `Map`, so a map column compares equal across libraries; this map detection is deliberately narrow — a `LargeList` of a two-field `key`/`value` struct whose key field is nullable, exactly polars' shape — so a genuine `list<struct{key, value}>` from another producer stays a list, with the residual ambiguity that a `LargeList` built by hand with that exact shape is read as a map. This is what makes `diff_tables(pl.DataFrame, pa.Table)` report identical results whether a string column arrives as polars' `Utf8View` or pyarrow's `Utf8`. Column names must be unique on each side; a repeated name raises `ValueError`. `diff.schema_arrow` is the same result as an Arrow table: it implements `__arrow_c_stream__`, so `polars.DataFrame(diff.schema_arrow)` and `pandas` consume it directly, and `diff.schema_arrow.to_pyarrow()` returns a `pyarrow.Table`.
